...
async def call_model_with_source_context(
    ...
    # Use ainvoke with config to support streaming events
...
async def get_graph_memory():
    """Get or initialize memory lazily"""
    global _memory
    if _memory is None:
        _memory = await get_memory()
    return _memory
...
# Graph will be created with memory on first use
source_chat_graph = None


async def get_source_chat_graph():
    """Get or initialize source chat graph lazily"""
    global source_chat_graph
    if source_chat_graph is None:
