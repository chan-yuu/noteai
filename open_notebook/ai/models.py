...
class Model(ObjectModel):
    ...
    @classmethod
    async def get_by_name_and_provider(cls, name: str, provider: str):
...
class ModelManager:
    ...
    async def get_model(self, model_id: str, **kwargs) -> Optional[ModelType]:
        ...
        # Handle named OpenAI-compatible providers (国内API服务商)
        provider = model.provider
        if provider in ["dashscope", "siliconflow", "volcengine"]:
            # Convert to openai-compatible and inject the correct base_url and api_key
            import os
            ...
            if not base_url or not api_key:
                raise ValueError(
                    ...
                    f"Please set {provider.upper()}_BASE_URL and {provider.upper()}_API_KEY environment variables."
                )
