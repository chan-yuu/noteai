...
export const sourceChatApi = {
  ...
  sendMessage: (sourceId: string, sessionId: string, data: SendMessageRequest) => {
    ...
    // IMPORTANT: For streaming, we MUST connect directly to the backend API
    // Next.js rewrites buffer the response, breaking streaming
    const apiUrl = typeof window !== 'undefined'
      ? (window as any).__NEXT_PUBLIC_API_URL__ || 'http://localhost:5055'
      : 'http://localhost:5055'

    const url = `${apiUrl}/api/sources/${sourceId}/chat/sessions/${sessionId}/messages`

    console.log('[Source Streaming] Direct API URL:', url)
