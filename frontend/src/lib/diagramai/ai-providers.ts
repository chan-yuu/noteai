...
import type { ProviderName } from "@/lib/diagramai/types/model-config"
 ...
 *
 * Environment variables:
 * - AI_PROVIDER: The provider to use (bedrock, openai, anthropic, google, azure, ollama, openrouter, deepseek, dashscope, siliconflow, sglang, gateway, modelscope)
 ...
 * - DASHSCOPE_BASE_URL: DashScope endpoint (optional, defaults to https://dashscope.aliyuncs.com/compatible-mode/v1)
...
export function getAIModel(overrides?: ClientOverrides): ModelConfig {
    ...
    switch (provider) {
        ...
        case "dashscope": {
            const apiKey = resolveApiKey(overrides, "DASHSCOPE_API_KEY")
            const serverBaseUrl = resolveBaseUrlEnv(
                overrides,
                "DASHSCOPE_BASE_URL",
            )
            ...
            const dashscopeProvider = createOpenAI({
                apiKey,
                baseURL,
            })
            model = dashscopeProvider.chat(modelId)
            break
        }
