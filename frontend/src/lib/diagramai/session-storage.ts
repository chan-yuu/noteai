...
// Constants
const DB_NAME = "diagram-ai"
const DB_VERSION = 2
const STORE_NAME = "sessions"
const MIGRATION_FLAG = "diagram-ai-migrated-to-idb"
...
export async function migrateFromLocalStorage(): Promise<string | null> {
    ...
    try {
        const savedMessages = localStorage.getItem("diagram-ai-messages")
        const savedXmlSnapshots = localStorage.getItem(
            "diagram-ai-xml-snapshots",
        )
        const savedXml = localStorage.getItem("diagram-ai-diagram-xml")
        ...
        if (savedMessages) {
            ...
            if (Array.isArray(messages) && messages.length > 0) {
                ...
                const session: ChatSession = {
                    ...
                    updatedAt: Date.now(),
                    messages: sanitized,
                    xmlSnapshots: savedXmlSnapshots
                        ? JSON.parse(savedXmlSnapshots)
                        : [],
                    diagramXml: savedXml || "",
                }
        ...
        if (migrationSucceeded) {
            ...
            localStorage.removeItem("diagram-ai-messages")
            localStorage.removeItem("diagram-ai-xml-snapshots")
            localStorage.removeItem("diagram-ai-diagram-xml")
        } else {
            console.warn(
