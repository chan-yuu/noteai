...
const getNavigationItems = (t: TranslationKeys) => [
  ...
  { name: 'AI绘图', action: 'diagramai', icon: PenTool, keywords: ['diagram', 'draw', 'flowchart', 'ai', 'drawing'] },
...
export function CommandPalette() {
  ...
  const { open: openDiagramAI } = useDiagramAIDialog()
        ...
        <CommandGroup heading={t.navigation.nav}>
          {navigationItems.map((item) => (
            <CommandItem
              ...
              onSelect={() => {
                if (item.action === 'diagramai') {
                  handleSelect(openDiagramAI)
                } else if (item.href) {
