...
import { ButtonWithTooltip } from "@/components/diagramai/button-with-tooltip"
import { ChatInput } from "@/components/diagramai/chat-input"
import { ModelConfigDialog } from "@/components/diagramai/model-config-dialog"
import { SettingsDialog } from "@/components/diagramai/settings-dialog"
import { useDiagram } from "@/contexts/diagram-context"
import { useDiagramToolHandlers } from "@/lib/hooks/diagramai/use-diagram-tool-handlers"
import { useDictionary } from "@/lib/hooks/diagramai/use-dictionary"
import { getSelectedAIConfig, useModelConfig } from "@/lib/hooks/diagramai/use-model-config"
import { useSessionManager } from "@/lib/hooks/diagramai/use-session-manager"
import { useValidateDiagram } from "@/lib/hooks/diagramai/use-validate-diagram"
import { getApiEndpoint } from "@/lib/diagramai/base-path"
import { formatMessage } from "@/lib/diagramai/i18n/utils"
import { isPdfFile, isTextFile } from "@/lib/diagramai/pdf-utils"
import { sanitizeMessages } from "@/lib/diagramai/session-storage"
import { STORAGE_KEYS } from "@/lib/diagramai/storage"
import type { UrlData } from "@/lib/diagramai/url-utils"
import { type FileData, useFileProcessor } from "@/lib/diagramai/use-file-processor"
import { useQuotaManager } from "@/lib/diagramai/use-quota-manager"
import { cn, formatXML, isRealDiagram } from "@/lib/diagramai/utils"
import type { ValidationState } from "./chat/ValidationCard"
import { ChatMessageDisplay } from "./chat-message-display"

// localStorage keys for persistence
const STORAGE_SESSION_ID_KEY = "diagram-ai-session-id"

// sessionStorage keys
const SESSION_STORAGE_INPUT_KEY = "diagram-ai-input"
...
export default function ChatPanel({
    ...
    useEffect(() => {
        fetch(getApiEndpoint("/api/diagramai/config"))
            .then((res) => res.json())
            .then((data) => {
    ...
    const { messages, sendMessage, addToolOutput, status, error, setMessages } =
        useChat({
            transport: new DefaultChatTransport({
                api: getApiEndpoint("/api/diagramai/chat"),
            }),
