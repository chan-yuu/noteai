...
declare global {
    interface Window {
        electronAPI?: {
            isElectron?: boolean
            getProxy?: () => Promise<{ httpProxy?: string; httpsProxy?: string }>
            setProxy?: (config: { httpProxy?: string; httpsProxy?: string }) => Promise<{ success: boolean; error?: string }>
            setUserLocale?: (lang: string) => Promise<void>
...
import { i18n, type Locale } from "@/lib/diagramai/i18n/config"
...
export const STORAGE_ACCESS_CODE_KEY = "diagram-ai-access-code"
const STORAGE_ACCESS_CODE_REQUIRED_KEY = "diagram-ai-access-code-required"
...
function SettingsContent({
    ...
    useEffect(() => {
        ...
        if (window.electronAPI?.getProxy) {
            window.electronAPI.getProxy().then((config: { httpProxy?: string; httpsProxy?: string }) => {
...
const changeLanguage = (lang: string) => {
        ...
        localStorage.setItem("diagram-ai-locale", lang)

        // 触发自定义事件通知语言已改变,DictionaryProvider会监听此事件
        window.dispatchEvent(new CustomEvent('diagramai-locale-change', { detail: { locale: lang } }))

        // Notify Electron main process to update its menu language
        if (window.electronAPI?.setUserLocale) {
            window.electronAPI.setUserLocale(lang).catch((error: Error) => {
                console.error("Failed to sync locale with Electron:", error)
            })
        }

        // 不再进行路由跳转,只保存语言设置并触发更新
    }
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch(
                getApiEndpoint("/api/diagramai/verify-access-code"),
                {
                    method: "POST",
                    headers: {
