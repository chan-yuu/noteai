...
def _check_named_openai_compatible_support(provider_name: str) -> bool:
    """
    Check if a named OpenAI-compatible provider is available.
    
    Args:
        provider_name: Provider name (e.g., 'dashscope', 'siliconflow', 'volcengine')
    
    Returns:
        bool: True if BASE_URL and API_KEY are both set
    """
    base_url_key = f"{provider_name.upper()}_BASE_URL"
    api_key_key = f"{provider_name.upper()}_API_KEY"
    return (
        os.environ.get(base_url_key) is not None
        and os.environ.get(api_key_key) is not None
    )
...
async def get_provider_availability():
    ...
    try:
        ...
        provider_status = {
            ...
            # Named OpenAI-compatible providers (国内便宜的API服务商)
            "dashscope": _check_named_openai_compatible_support("dashscope"),
            "siliconflow": _check_named_openai_compatible_support("siliconflow"),
            "volcengine": _check_named_openai_compatible_support("volcengine"),
        }
        ...
        for provider in available_providers:
            ...
            # Special handling for named OpenAI-compatible providers
            elif provider in ["dashscope", "siliconflow", "volcengine"]:
                # These providers support the same types as openai-compatible
                for model_type in ["language", "embedding", "speech_to_text", "text_to_speech"]:
                    if (
                        model_type in esperanto_available
                        and "openai-compatible" in esperanto_available[model_type]
                    ):
                        supported_types[provider].append(model_type)
